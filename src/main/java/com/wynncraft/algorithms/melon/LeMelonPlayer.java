...
@Information(name = "LeMelon Player", version = 1, authors = "Melon")
public final class LeMelonPlayer implements IPlayer {
    ...
    private LeMelonPlayer(List<IEquipment> equipment, int[] allocated) {
    ...
    public static final class Builder implements IPlayerBuilder<LeMelonPlayer> {

        private final List<IEquipment> equipment = new ArrayList<>(24);
        private final int[] allocated = new int[SKILL_POINTS.length];

        @Override
        public IPlayerBuilder<LeMelonPlayer> equipment(IEquipment... items) {
            Collections.addAll(equipment, items);
            return this;
        }

        @Override
        public IPlayerBuilder<LeMelonPlayer> allocate(SkillPoint point, int amount) {
            allocated[point.ordinal()] = amount;
            return this;
        }

        @Override
        public LeMelonPlayer build() {
            return new LeMelonPlayer(equipment, allocated);
        }
    }
}
