...
@Information(name = "LeSprint Player", version = 1, authors = "Melon")
public final class LeSprintPlayer implements IPlayer {
    ...
    private LeSprintPlayer(List<IEquipment> equipment, int[] allocated) {
    ...
    public static final class Builder implements IPlayerBuilder<LeSprintPlayer> {

        private final List<IEquipment> equipment = new ArrayList<>(16);
        private final int[] allocated = new int[SKILL_POINTS.length];

        @Override
        public IPlayerBuilder<LeSprintPlayer> equipment(IEquipment... items) {
            Collections.addAll(equipment, items);
            return this;
        }

        @Override
        public IPlayerBuilder<LeSprintPlayer> allocate(SkillPoint point, int amount) {
            allocated[point.ordinal()] = amount;
            return this;
        }

        @Override
        public LeSprintPlayer build() {
            return new LeSprintPlayer(equipment, allocated);
        }
    }
}
